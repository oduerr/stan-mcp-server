#!/usr/bin/env python3
"""Evaluate model.stan on the Synthetic Groups (sparse hierarchical) dataset."""
...
def main():
    ...
    all_group_ids = sorted(set(int(r["group_id"]) for r in train_rows + test_rows))
    J = len(all_group_ids)
    ...
    stan_data = {
        ...
        "J": J,
        "group_train": [int(r["group_id"]) for r in train_rows],
        "group_test":  [int(r["group_id"]) for r in test_rows],
        "value_train": [float(r["value"]) for r in train_rows],
        "value_test":  [float(r["value"]) for r in test_rows],
    }
