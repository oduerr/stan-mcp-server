#!/usr/bin/env python3
"""Evaluate model.stan on the 1D Regression (Large) dataset.

Handles compilation, sampling, NLPD computation, and all bookkeeping.
Uses 5x more MCMC iterations than the small version for lower Monte Carlo noise.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate model.stan on 1D Regression (Large) dataset")
    ...
    stan_data = {
        ...
        "predictor_train": [float(r["predictor"]) for r in train_rows],
        "predictor_test": [float(r["predictor"]) for r in test_rows],
    ...
    # Sample: 4 chains x 5000 iterations (30x more for low MC noise)
    print("Sampling: 4 chains x 5000 iterations (5000 warmup)...")
