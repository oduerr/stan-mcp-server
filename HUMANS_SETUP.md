...
## Claude Desktop (no server to run)

Claude Desktop has no shell access, so it cannot install the server for you the
way Claude Code can — this part is manual, and the config is only picked up
when Desktop restarts. The upside: you never start the server yourself. Desktop
launches it over **stdio** when it opens and shuts it down with the app.

After the install above, open Settings → Developer → Edit Config and add:
...
## Start the server (for HTTP clients like Claude Code)
...
stan-mcp-server \
  ...
  --transport streamable-http   # or 'stdio' — Claude Desktop, locally or over SSH
```

You should use the TOKEN when it is accessible from the outside.
...
[Claude Desktop](#claude-desktop-no-server-to-run) above.
...
command do the hop, e.g. `ssh user@host /path/to/stan-mcp-server --transport stdio --datasets-dir … --results-dir …`, or run a local stdio↔HTTP bridge.
See [What stdio changes](#what-stdio-changes) above.
