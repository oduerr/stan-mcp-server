...
## Transports, in one line

`streamable-http` (default) serves the MCP endpoint plus the HTTP sidecar;
`stdio` runs as a client-launched subprocess with **no sidecar** — uploads
become file copies, no train-download URL is advertised, and `--token` is
refused. The five connection topologies and their trade-offs are in
[HUMANS_SETUP.md](../HUMANS_SETUP.md#how-to-connect).
